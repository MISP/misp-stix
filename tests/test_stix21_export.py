...
class TestSTIX21EventExport(TestSTIX21GenericExport):
    ...
    def _test_event_with_escaped_characters(self, event):
        *initial_attributes, _ = deepcopy(event['Attribute'])
        misp_objects = deepcopy(event['Object'])
        self.parser.parse_misp_event(event)
        *attributes, email_message_id = self.parser._misp_event.attributes
        stix_objects = self._check_bundle_features(142)
        indicators = [obj for obj in stix_objects if obj.type == 'indicator']
        email_message_id_indicator = indicators.pop(19)
        self._check_spec_versions(stix_objects)
...
class TestSTIX21ExportInteroperability(TestSTIX2Export, TestSTIX21):
    def setUp(self):
        self.parser = MISPtoSTIX21Parser(use_cti_uuids=True)
    ...
    def _run_galaxy_tests(self, event, expected_stix_id):
        ...
        for stix_object in stix_objects:
            self.assertEqual(stix_object.spec_version, '2.1')
        identity, grouping = stix_objects
        identity_id = self._check_identity_features(identity, orgc, timestamp)
        object_ref = self._check_grouping_features(grouping, identity_id)[0]
