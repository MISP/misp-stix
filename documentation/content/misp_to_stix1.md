...
Now we can have a look at the results when we add attributes, objects, galaxies, or tags (**The detailed mapping for each structure type is available below**).
...
Those two last examples were very simple to have an overview of what happens, for more details the detailed attributes mapping and objects mapping are available below.

For the next detailed mappings, we will just display the resulting `Indicators`, `Observables`, `TTPs`, `Courses of Action`, `Threat Actors`, and so on, but again keep in mind they are embedded within their corresponding structure (`Incident`, `STIX Package`, etc.)
